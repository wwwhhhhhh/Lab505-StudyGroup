...
class Decoder(nn.Module):
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        if self.training:
            ...
            diffY = h4.size()[2] - h3.size()[2]
            diffX = h4.size()[3] - h3.size()[3]
            h3 = F.pad(h3, [diffX // 2, diffX - diffX // 2,
                          diffY // 2, diffY - diffY // 2])

            diffY = h3.size()[2] - h2.size()[2]
            diffX = h3.size()[3] - h2.size()[3]
            h2 = F.pad(h2, [diffX // 2, diffX - diffX // 2,
                            diffY // 2, diffY - diffY // 2])
...
class UNetFormer(nn.Module):
    def __init__(
        ...
        self.conv = nn.Conv2d(8, 3, kernel_size=1, bias=False)

        self.backbone = timm.create_model(
            backbone_name, features_only=True, output_stride=32, out_indices=(1, 2, 3, 4), pretrained=pretrained
        )
...
if __name__ == "__main__":
    # conv = nn.Conv2d(4, 3, kernel_size=1, bias=False)
    # a = torch.rand(4, 512, 512)
    # c = conv(a)
    # print(c.shape)

    net = UNetFormer()
    a = torch.rand(1, 1, 1000, 1000)
    out = net(a)
    # print(out.shape)
